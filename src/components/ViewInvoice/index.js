    ...
    return(
      <div className="popupdelete">
        <div className="box-delete">
          ...
          <p className="description">Are you sure you want to delete invoice {id}? this action cannot  be undone</p>
          <div className="box-button">         
            <button className="btn delete-non" onClick = {togglePrompt} >Cancel</button>
            <button className="btn delete-yes" onClick = {Delete} >Delete</button>
          </div>
        </div>
      </div>
    )
      ...
      <div className="invoice-content">
            <div className="header-invoice-information row justify-content-between">
              ...
              <div className="adress-information">
                <p className="body2 adress-number">{invoice.senderAddress.street}</p>
              ...
              <div className="bill-form">
                ...
                <form action="" onSubmit = {Submit}>
                  ...
                  <div className="data-control">
                    ...
                    <input type="text" defaultValue ={invoice.clientAddress.street}  name="street" onChange={(ev) => handleChange(ev,'clientAddress')}  />
                  </div>
                  <div className="data-control row justify-content-between ">
                    <div className="data-input">
                      <label className="body1">City</label>
                      <input type="text" defaultValue ={invoice.clientAddress.city} name = "city" onChange={(ev) => handleChange(ev,'clientAddress')} />
                    </div>
  
                    <div className="data-input">
                      <label className="body1">Post Code</label>
                      <input type="text" defaultValue ={invoice.clientAddress.postCode} name="postCode" onChange={(ev) => handleChange(ev,'clientAddress')}  />
                    </div>
  
                    <div className="data-input">
                      <label className="body1">Country</label>
                      <input type="text" defaultValue ={invoice.clientAddress.country} name="country" onChange={(ev) => handleChange(ev,'clientAddress')}  />
                    </div>
                  </div>
                  ...
                  <div className="data-control">
                    ...
                    <input type="text" defaultValue ={invoice.clientName} name="clientName" onChange={(ev) => handleChange(ev,'')} />
                  </div>
                  <div className="data-control">
                    <label className="body1">Street adress</label>
                    <input type="text" defaultValue = {invoice.senderAddress.street} name="street" onChange={(ev) => handleChange(ev,'senderAddress')} />
                  </div>
                  <div className="data-control row justify-content-between">
                    <div className="data-input">
                      <label className="body1">City</label>
                      <input type="text" defaultValue = {invoice.senderAddress.city} name="city" onChange={(ev) => handleChange(ev,'senderAddress')} />
                    </div>
  
                    <div className="data-input">
                      <label className="body1">Post Code</label>
                      <input type="text" defaultValue = {invoice.senderAddress.postCode} name = "postCode" onChange={(ev) => handleChange(ev,'senderAddress')} />
                    </div>
  
                    <div className="data-input">
                      <label className="body1">Country</label>
                      <input type="text" defaultValue = {invoice.senderAddress.country} name="country" onChange={(ev) => handleChange(ev,'senderAddress')} />
                    </div>
                  </div>
                  ...
                  <div className="data-control row justify-content-between">
                    <div className="date-control">
                      ...
                      <input type="date" defaultValue = {invoice.createdAt} name="createdAt" onChange={(ev) => handleChange(ev,'')} />
                    </div>
                    <div className="payement-terms">
                      <label className="body1">Payement terms</label>
                      <input type="date" defaultValue = {invoice.paymentDue} name="paymentDue" onChange={(ev) => handleChange(ev,'')} />
                    </div>
                  </div>
  
                  <div className="data-control">
                    <label className="body1">Project description</label>
                    <input type="text" defaultValue = {invoice.description} name = "description" onChange={(ev) => handleChange(ev,'')} />
                  </div>
                  ...
                  <div className="list-items">
                    ...
                    {invoice.items.map((elem , index)=>{
                      return(
                        <div key={index} className="list-item row justify-content-between">
                          <input className="item-name" type="text" defaultValue = {elem.name} />
                          <div className="price-control row justify-content-between align-items-center">
                            <input className="item-qty" type="text" defaultValue = {elem.quantity} />
                            <input className="item-price" type="number" defaultValue = {elem.price}/>
                    ...
                    <div className="btn-control-edit">
                     <button className="btn cancel-btn" onClick={toggleDisplayEdit} >Cancel</button>
    ...
    return(
        <div className = "nav-edit-invoice row align-items-center justify-content-between" >
          <div className = "content-status row align-items-center justify-content-between">
            ...
            <h3 className ={`status ${invoice.status === "paid"? "paid":invoice.status === "draft"?  "draft":"pending"} center`}>{invoice.status}</h3>
          </div>
          <div className = {`control-invoice row align-items-center justify-content-between ${invoice.status === "draft"? "none" : ""}`}>
            <button className = "btn edit center" onClick = {toggleDisplayEdit} >Edit</button>
            <button className = "btn delete center" onClick = {togglePrompt} >Delete</button>
            <button className = {`btn mark-as center ${invoice.status === "draft"? "none" : ""}`} onClick = {() => markAs(invoice.status)}>{`Mark as ${invoice.status === "pending" ? "paid" :invoice.status === "paid" ? "draft" : ''}`}</button>
          </div>
        </div>
    )
  }
